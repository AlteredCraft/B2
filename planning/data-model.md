...
and the **filesystem is authoritative** for it; B2 proxies the OS rather than modeling folders: create is
`mkdir -p` (`Vault::create_dir`), move is one `rename` (`move_dir`), delete is `remove_dir_all`
...
is one-to-one with the filesystem *by construction*, in both directions — a Finder `mkdir` appears on the
next pulse, and a folder emptied by a move stays visible until the human removes it. The "no durable
