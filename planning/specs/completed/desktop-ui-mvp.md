...
| Switch vault (folder icon, top bar) — native folder picker; repoints the app at another vault | **no façade op** — host state: `choose_vault` swaps `AppState`'s runtime root (root resolution is the host's job, main.rs), then every later command opens over the new root; the pick is also **remembered** (persisted to `<data-dir>/b2/last-vault`) so the next launch reopens it — startup precedence: launch arg › remembered pick › `$B2_VAULT_PATH` | added post-MVP |
