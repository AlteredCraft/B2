...
- **Folders are fs-authoritative, empty folders first-class (2026-07-21)** — the file tree is now
  **one-to-one with the vault's managed (non-dot) directory tree in both directions**: folders are
  ...
  off disk. Two new façade ops —
  `Vault::list_dirs` (a live fs walk, dot-folders skipped; the tree's structure half, so a Finder `mkdir`
  or a folder emptied by a move stays visible) and `Vault::create_dir` (a real on-disk create, missing
  parents included, occupied targets refused; the desktop's
...
| Theme | Issues |
|---|---|
| **Desktop** (beyond editing) | graph visualization [#22](https://github.com/AlteredCraft/B2/issues/22) · packaging/signing/distribution [#23](https://github.com/AlteredCraft/B2/issues/23) · auto-index-on-open (split §9) [#25](https://github.com/AlteredCraft/B2/issues/25) **shipped 2026-07-16** (frontend-only, no confirm dialog: `autoIndexOnOpen` in `ui/src/main.ts` runs on boot + vault switch off the `notes_embedded/total` detector — `project` a never-projected vault then `embed`, or resume embedding a partly-embedded one; embed skipped without a model; silent, progress-meter-only) |
| **Indexing & performance** | "semantic: N/M embedded" signal (split §9) [#26](https://github.com/AlteredCraft/B2/issues/26) **shipped 2026-07-14** (`Vault::embed_status` + `db::embed_progress`, model-free; `vault_info` carries the fraction, the desktop search caveat flags "keyword-only for now (N/M embedded)") · relevance-ordered embedding (split §9) [#27](https://github.com/AlteredCraft/B2/issues/27) · cross-process CLI reindex + `b2 status` + Ctrl-C (async §8) [#16](https://github.com/AlteredCraft/B2/issues/16) **partially shipped 2026-07-18** (`b2 status` over `Vault::embed_status`; foreground **Ctrl-C** cooperative-cancel wiring the shipped `ControlFlow` seam to a SIGINT flag; a **single-in-flight** advisory lock at `.b2/reindex.lock`. Detach leans on shell job control — `b2 reindex &` / `nohup` — since WAL already lets `status`/`search` read live and the lock keeps a second run out; built-in `--detach` daemonization split to [#55](https://github.com/AlteredCraft/B2/issues/55)) · faster/smaller embedder spike (async §8) [#17](https://github.com/AlteredCraft/B2/issues/17) |
| **Engine & quality** | property tests for the invariants [#18](https://github.com/AlteredCraft/B2/issues/18) · qmd chunker upgrade [#19](https://github.com/AlteredCraft/B2/issues/19) **shipped 2026-07-13, closed** (`chunk.rs`, [specs/completed/qmd-chunker.md](specs/completed/qmd-chunker.md) Steps 1–2: size-targeted + overlapping + `heading_path`, `ChunkConfig` levers, unit-tested); the real-model eval gate (spec §7, out-of-CI) is split out as [#44](https://github.com/AlteredCraft/B2/issues/44) — its **"make the gate sensitive first" groundwork shipped 2026-07-19** ([specs/eval-strategy.md](specs/eval-strategy.md): clustered + long-note corpus, passage-level ranks via `Vault::search_chunks`, BM25-vs-hybrid lift, a `similar` label set, the in-process `ChunkConfig` sweep (`just eval-sweep`), `results.jsonl`), leaving #44 the before/after run itself · distance-weighting for `b2 similar` [#20](https://github.com/AlteredCraft/B2/issues/20) · **deferred refactor:** the kernel's free-function ops (`add.rs`/`mv.rs`/`ingest.rs`) now share a five-wide Vault-collaborator prefix (`conn, idgen, cfg, embedder, vault_root`) behind three `#[allow(clippy::too_many_arguments)]`s — bundle it into a small borrowed context struct (the CLAUDE.md short-lived-view exception) **the next time one of these signatures needs another parameter**; don't add a sixth collaborator arg |
